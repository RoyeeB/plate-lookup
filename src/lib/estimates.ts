...
/** 0-100 km/h (s) from power-to-weight ratio. */
function estimateZeroToHundred(
  hp: number | null,
  weightKg: number | null
): number | null {
  if (hp === null || weightKg === null) return null;
  const ratio = weightKg / hp; // kg per hp
  const seconds = ratio * 0.9;
 ...
 */
export function estimateSpecs(record: VehicleRecordRaw): EstimatedSpec[] {
  ...
  const weight = estimateCurbWeight(displacement, fuel);
  const zeroTo100 = estimateZeroToHundred(hp, weight);
  ...
  if (weight !== null) {
    specs.push({ key: 'curbWeight', label: t.estimates.curbWeight, value: `${approx}${weight}` });
  }
  if (zeroTo100 !== null) {
    specs.push({
      key: 'zeroToHundred',
